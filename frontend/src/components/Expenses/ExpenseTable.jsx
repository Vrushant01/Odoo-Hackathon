...
export const ExpenseTable = ({
  ...
  const columns = [
    ...
    { accessorKey: "type", header: "Expense Type" },
    { accessorKey: "vendor", header: "Billing Vendor" },
    { accessorKey: "amount", header: "Amount" },
    { accessorKey: "paymentMethod", header: "Payment Method" },
    { accessorKey: "date", header: "Invoice Date" },
        ...
        <div className={styles.leftSection}>
          {selectedIds.length > 0 && (
            <div className={styles.bulkBar}>
              <span className={styles.bulkText}>{selectedIds.length} expenses selected</span>
              <Button
                variant="outline"
                size="sm"
                ...
                {table.getAllLeafColumns().map((column) => {
                  ...
                  return (
                    <label key={column.id} className={styles.visibilityItem}>
                      ...
                      />
                      {column.id === "id" ? "Expense ID" :
                       column.id === "vehicle" ? "Vehicle Unit" :
                       column.id === "tripId" ? "Trip ID" :
                       column.id === "type" ? "Expense Type" :
                       column.id === "vendor" ? "Billing Vendor" :
                       column.id === "paymentMethod" ? "Payment Method" :
                       column.id === "date" ? "Invoice Date" :
                       column.id === "status" ? "Payment Status" :
                       column.id}
                    </label>
                  );
                })}
      ...
      <div className={tableStyles.tableWrapper}>
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="No billing statements match your filter parameters."
          />
        ) : (
      ...
      {!isLoading && data.length > 0 && (
        <div className={tableStyles.pagination}>
          <div className={tableStyles.pageInfo}>
            Showing {pagination.pageIndex * pagination.pageSize + 1} to {Math.min((pagination.pageIndex + 1) * pagination.pageSize, totalCount)} of {totalCount} expenses
          </div>
