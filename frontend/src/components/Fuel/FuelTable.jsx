...
export const FuelTable = ({
  ...
  const columns = [
    ...
    { accessorKey: "quantity", header: "Volume" },
    { accessorKey: "costPerUnit", header: "Price/L" },
        ...
        <div className={styles.leftSection}>
          {selectedIds.length > 0 && (
            <div className={styles.bulkBar}>
              <span className={styles.bulkText}>{selectedIds.length} logs selected</span>
              <Button
                variant="outline"
                size="sm"
                  ...
                  return (
                    <label key={column.id} className={styles.visibilityItem}>
                      ...
                      {column.id === "id" ? "Log ID" :
                       ...
                       column.id === "tripId" ? "Trip ID" :
                       column.id === "fuelType" ? "Fuel Type" :
                       column.id === "costPerUnit" ? "Price/L" :
                       column.id === "fuelStation" ? "Fuel Station" :
                       column.id === "date" ? "Refuel Date" :
                       column.id}
                    </label>
                  );
                })}
      ...
      <div className={tableStyles.tableWrapper}>
        {isLoading ? (
          <div className={tableStyles.loadingOverlay}>
            ...
            <p>Loading refueling logs...</p>
          </div>
        ) : data.length === 0 ? (
          <EmptyState
            title="No fuel logs logged"
            description="No refueling logs match your filter parameters."
          />
        ) : (
                  ...
                  {table.getVisibleFlatColumns().map((column) => {
                    ...
                    return (
                      <td key={column.id} className={tableStyles.td}>
                        ...
                        ) : column.id === "totalCost" || column.id === "costPerUnit" ? (
                          formatCost(row[column.id])
                        ) : column.id === "quantity" ? (
      ...
      {!isLoading && data.length > 0 && (
        <div className={tableStyles.pagination}>
          <div className={tableStyles.pageInfo}>
            Showing {pagination.pageIndex * pagination.pageSize + 1} to {Math.min((pagination.pageIndex + 1) * pagination.pageSize, totalCount)} of {totalCount} logs
          </div>
